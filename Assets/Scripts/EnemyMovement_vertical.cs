...
public class EnemyMovement_vertical : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        actualPosition = transform.position.y;
        ...
        if (actualPosition < lastPosition) transform.localScale = new Vector3(-0f,0f, 0f);
        if (actualPosition > lastPosition) transform.localScale = new Vector3(0f, 0f, 0f);
